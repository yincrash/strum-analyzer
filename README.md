...
- **2:1 ambiguity: peak at N Hz (half)**: the pluck also shows a distinct peak at exactly half the reading. Two very different things look identical in audio: the reading is the 2× harmonic of a weaker fundamental (phone capture paths tuned for speech attenuate low notes, so the belt's true pitch is the half value), or the belt's longer span is ringing at half the pitch (likely with the gantry 150 mm from the tensioner on a roughly 300 mm axis, and then the reading is right). To settle it, damp the long span with a finger and pluck again: if the half peak vanishes, the reading stands; if it stays, the belt's pitch is the half value. Moving the gantry to 100 mm also separates them: a harmonic rises with the reading, a long span barely moves.
